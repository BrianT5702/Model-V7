...
const CeilingManager = ({ projectId, canEdit = true, onClose, onCeilingPlanGenerated, updateSharedPanelData = null, sharedPanelData = null }) => {
    ...
    const [isUpdatingExcludeCeiling, setIsUpdatingExcludeCeiling] = useState(false);
    ...
    const roomsAvailableForMerge = useMemo(() => {
        ...
        return filteredRooms.filter(room =>
            (!room.ceiling_zones || room.ceiling_zones.length === 0) && !room.exclude_from_ceiling
        );
    }, [filteredRooms]);
    ...
    const notifySupportOptionsUserChange = useCallback(() => {
    ...
    const handleExcludeFromCeilingToggle = async (room, excludeFromCeiling) => {
        ...
        setError(null);
        try {
            await api.patch(`/rooms/${room.id}/`, { exclude_from_ceiling: excludeFromCeiling });
            setAllRooms(prev =>
                prev.map(r => (r.id === room.id ? { ...r, exclude_from_ceiling: excludeFromCeiling } : r))
            );
            if (excludeFromCeiling) {
                setCeilingPanels(prev => prev.filter(panel => panel.room_id !== room.id));
                setCeilingPlans(prev =>
                    prev.filter(plan => plan.room !== room.id && plan.room_id !== room.id)
                );
                await loadExistingCeilingPlan();
            }
        } catch (err) {
            ...
            const message =
                ...
                detail?.exclude_from_ceiling?.[0] ||
                detail?.detail ||
                'Failed to update room ceiling setting.';
            setError(message);
        } finally {
            setIsUpdatingExcludeCeiling(false);
        }
    };
    ...
    const processCeilingPanelsForSharing = (panels, rooms) => {
        ...
        const excludedIds = new Set(
            (roomList || []).filter(room => room.exclude_from_ceiling).map(room => room.id)
        );
        const activePanels = excludedIds.size
            ? panels.filter(panel => !panel?.room_id || !excludedIds.has(panel.room_id))
            : panels;
    ...
    const generateCeilingPlanForRoom = async (roomId, config, options = {}) => {
        ...
        if (targetRoom?.exclude_from_ceiling) {
            setError(`"${targetRoom.room_name}" is marked as not requiring a ceiling. Turn off that option to generate panels.`);
            return;
        }
    ...
    const ceilingPanelsMap = useMemo(() => {
        ...
        if (ceilingPanels && Array.isArray(ceilingPanels)) {
            ceilingPanels.forEach(panel => {
                ...
                if (roomId) {
                    const room = allRooms.find(r => r.id === roomId);
                    if (room?.exclude_from_ceiling || excludedCeilingRoomIds.has(roomId)) {
                        return;
                    }
                                ...
                                >
                                    ...
                                    {filteredRooms.map(room => (
                                        <option key={room.id} value={room.id}>
                                            {room.room_name}
                                            {room.exclude_from_ceiling ? ' (no ceiling)' : ''}
                                        </option>
                                    ))}
                                </select>
                            </div>
                                    ...
                                    {showRoomDetailsPanel ? (
                                        ...
                                        showRoomDetails ? (
                                                ...
                                                <label className="flex items-start gap-3 p-3 rounded-lg border border-amber-200 bg-amber-50/60 cursor-pointer">
                                                    ...
                                                    <span>
                                                        <span className="block text-sm font-medium text-amber-900">
                                                            No ceiling for this room
                                                        </span>
                                                        <span className="block text-xs text-amber-800/90 mt-0.5">
                                                            Skip ceiling generation and remove existing ceiling panels for this room.
                                                        </span>
                                                    </span>
                                                </label>
                                            ...
                                            <button
                                                ...
                                                title={selectedRoom?.exclude_from_ceiling ? 'This room is excluded from ceiling generation' : undefined}
                                                className={`px-6 py-2 text-sm rounded-lg font-medium transition-colors shadow-md ${
                                                    isRegeneratingRoom || selectedRoom?.exclude_from_ceiling
